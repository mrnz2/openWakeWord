...
- **Instalacja pakietów:** `python colab/install_colab_deps.py` — `torch`/`torchaudio` (CUDA cu124 → fallback PyPI), potem **każda linia** z `requirements-colab-train.txt` i `requirements-colab-tflite.txt` osobnym `pip install` (w logu widać dokładnie który pakiet padł). W notebooku komórka **apt** musi mieć `build-essential` i `python3-dev` (kompilacja m.in. `webrtcvad`). **Nie** instalujemy `openwakeword` z PyPI; trening z **klonu** v0.6.0.
