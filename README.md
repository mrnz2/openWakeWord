...
- **Instalacja pakietów:** w notebookzie używane jest `runpy.run_path("colab/install_colab_deps.py")` (ten sam proces co kernel — pełny log w komórce). Skrypt: `torch`/`torchaudio` (cu124 → PyPI), potem linie z `requirements-colab-train.txt`, potem **`tensorflow-cpu`** + `onnx2tf` (mniej konfliktów z PyTorch CUDA niż pełny `tensorflow`). **webrtcvad:** najpierw PyPI, przy błędzie build z GitHub `wiseman/py-webrtcvad`. Apt: `build-essential`, `python3-dev`. **Nie** instalujemy `openwakeword` z PyPI.
